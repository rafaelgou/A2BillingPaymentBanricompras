...
getpost_ifset (array('status', 'idpedido', 'identpedido', 'VlrTotal'));

write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT BANRICOMPRAS: identpedido=$identpedido | VlrTotal=$VlrTotal | status=$status\n".print_r($_REQUEST, true));

if ($identpedido=="" || $idpedido=="") {
	write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT BANRICOMPRAS: identpedido=$identpedido"." NO TRANSACTION ID PROVIDED IN REQUEST");
  header('Location: checkout_success.php?errcode=-2');  exit();
}

// Retirada checagem, não utilizado
//if ($VlrTotal=="") {
//	write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT BANRICOMPRAS: VlrTotal=$VlrTotal"." NO VALUE ID PROVIDED IN REQUEST");
//  header('Location: checkout_success.php?errcode=-2');  exit();
//}

if ($status=="") {
	write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT BANRICOMPRAS: status=$status"." NO ERRCODE ID PROVIDED IN REQUEST");
  header('Location: checkout_success.php?errcode=-2');  exit();
}
...
if(!is_array($transaction_data) && count($transaction_data) == 0) {
	write_log(LOGFILE_EPAYMENT, basename(__FILE__).
		' line:'.__LINE__." EPAYMENT BANRICOMPRAS: transactionID=$transactionID"." ERROR INVALID TRANSACTION ID PROVIDED, TRANSACTION ID =".$transactionID);
	 header('Location: checkout_success.php?errcode=-2');  exit();
} else {
	write_log(LOGFILE_EPAYMENT, basename(__FILE__).
		' line:'.__LINE__." EPAYMENT BANRICOMPRAS: transactionID=$transactionID"." EPAYMENT RESPONSE: TRANSACTIONID = ".$transactionID.
...
{
	write_log(LOGFILE_EPAYMENT, basename(__FILE__).
		' line:'.__LINE__."  EPAYMENT BANRICOMPRAS: transactionID=$transactionID"." ERROR NO AUTH BANRICOMPRAS CHECK CODREDE / CODESTAB / SENHA");
...
if (count($data->dados[0]))
{
  if ( ! in_array(
    ...
    write_log(LOGFILE_EPAYMENT, basename(__FILE__).
      ' line:'.__LINE__."  EPAYMENT BANRICOMPRAS: transactionID=$transactionID"." TRANSACTION NOT AUTHORIZED");
    $security_verify = false;
  }
// Não existe o pedido no Banricompras, aborta          
} else {
	write_log(LOGFILE_EPAYMENT, basename(__FILE__).
		' line:'.__LINE__."  EPAYMENT BANRICOMPRAS: transactionID=$transactionID"." ERROR NO DATA BANRICOMPRAS FOR THIS IdPedido OR ");
	$security_verify = false;
}
...
if ($paymentmethod == 'banricompras_pgta' && $security_verify == false) 
{
  try {
...
{
  case 'banricompras_pgta':
    if ($status == 1) $status = 2;
    break;

  case 'banricompras_pgbc':
    break;

  default:
    write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT BANRICOMPRAS: paymentmethod=$paymentmethod"." UNSUPPORTED PAYMENT METHOD IN THIS SCRIPT - BAD REQUEST");
  	header('Location: checkout_success.php?errcode=-2');  exit();    
    break;
}
...
header('Location: checkout_success.php?errcode=' . $status);